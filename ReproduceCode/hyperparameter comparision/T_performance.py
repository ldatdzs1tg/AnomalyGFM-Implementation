...
records = []
for T in range(200, 550, 100):
    print("="*30)
    print(f'Training with embedding dim: {T}')
    ...
    for epoch in tqdm(range(EPOCHS+1)):
        ...
        normal_prompt_raw = torch.randn(T).to(DEVICE)
        abnormal_prompt_raw = torch.randn(T).to(DEVICE)
...
df.to_csv(str(SCRIPT_DIR / "T_results.csv"), index=False)
print("\nResults saved to T_results.csv")