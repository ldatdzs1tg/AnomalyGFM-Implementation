...
DATASET_DIR = SCRIPT_DIR.parent.parent / "datasets for AnomalyGFM"
...
# Load data
sample_feature = np.load(str(DATASET_DIR / f'{DATASET}_feature_{SUBGRAPH_SIZE+1}_1.npy'))
sample_labels = np.load(str(DATASET_DIR / f'{DATASET}_label_{SUBGRAPH_SIZE+1}_1.npy'))
...
# Load lại setting
model = torch.load(SCRIPT_DIR / "model_residual2.pth", map_location=DEVICE)
model.eval()
