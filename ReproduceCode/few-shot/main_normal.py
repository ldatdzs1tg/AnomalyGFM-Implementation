...
from model import normalFewshotModel
...
DATASET_TEST = "amazon_svd"     # Target dataset để prompt tune + test
...
K_SHOT = 1  # Số lượng mẫu few-shot để fine-tune prompt
EPOCHS = 10

DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
SEED = 0

if DATASET_TEST in ['amazon_svd',  'Amazon_upu_svd', 'Disney_svd', 'reddit_svd']:  # small datasets
    EPOCHS = 5
elif DATASET_TEST in ['tolokers_svd',  'yelp_svd', 'questions_svd']:  # middle datasets
    EPOCHS = 10
elif DATASET_TEST in ['elliptic_svd',  'tf_finace_svd']:  # large datasets
    EPOCHS = 15   
...
all_idx_test_normal = list(np.where(ano_labels_test == 0)[0])

random.shuffle(all_idx_test)
random.shuffle(all_idx_test_normal)
# few-shot id for training
few_shot_train_id = all_idx_test_normal[:K_SHOT]
...
print(f"Tuning with {len(few_shot_train_id)} labeled Normal nodes.")
...
model = normalFewshotModel(input_dim=INPUT_DIM,
...
for epoch in range(EPOCHS+1):
    ...
    diff_normal = torch.sqrt(torch.sum((residual_few - p_n) ** 2, dim=1))

    loss_pt = torch.mean(diff_normal)
    loss = loss_pt
